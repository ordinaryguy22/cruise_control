...
def main():
    ...
    show_countdown('Lane and Object Detection')
...
def generate_final_mask(frame):
    ...
    # Apply masks only on ROI
    yellow_mask = cv2.inRange(hsv_roi, lower_yellow, upper_yellow) | \
                  cv2.inRange(hsv_roi, lower_white, upper_white)

    # Apply white pixel restriction on ROI
    restricted_mask = restrict_white_pixel_band_center(yellow_mask)

    # Create full-size mask and insert processed ROI
    final_mask = np.zeros_like(frame_resized[:, :, 0])  # Single channel
...
def draw_lane_overlay(frame, left_points, right_points):
    ...
    # Create polygon covering the area between the lines
    polygon_pts = np.array([
...
def show_countdown(window_name, duration=5):
    """Display a countdown before starting processing"""
    blank_frame = np.zeros((360, 640, 3), dtype=np.uint8)
    for i in range(duration, 0, -1):
        countdown_frame = blank_frame.copy()
        cv2.putText(countdown_frame, f"Starting in: {i}", (150, 180),
